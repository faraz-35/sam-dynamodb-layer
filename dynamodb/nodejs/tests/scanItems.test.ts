...
describe('DynamoDB Scan Items', () => {
    ...
    test('scanItems should retrieve items from the DynamoDB table starting from the exclusiveStartKey', async () => {
        ...
        const secondResult = await scanItems(1, firstResult?.Items?.[0].id, undefined, tableName);
    ...
    test('scanItems should retrieve items with specific attributes from the DynamoDB table', async () => {
        const result = await scanItems(undefined, undefined, 'id,testId', tableName);

        expect(result?.Items).toBeDefined();
        result?.Items?.forEach((item) => {
            expect(Object.keys(item)).toEqual(expect.arrayContaining(['id', 'testId']));
        });
    });

    test('scanItems should retrieve items that match the filter from the DynamoDB table', async () => {
        const filter = { testId: 'TestItem' };
        const result = await scanItems(undefined, undefined, undefined, tableName, filter);
    ...
    test('scanItems should populate fields from another table', async () => {
        ...
        const result = await scanItems(undefined, undefined, undefined, 'Test2Table', undefined, populate);

        expect(result?.Items).toBeDefined();
        result?.Items?.forEach((item) => {
            expect(item.referenceId).toHaveProperty('id');
            expect(item.referenceId).toHaveProperty('testId');
        });
    });
    ...
    test('scanItems should filter items based on the populateFilter', async () => {
        ...
        const populateFilter = [{ fieldName: 'referenceId', filter: { id: '123' } }];
        ...
        expect(result?.Items).toBeDefined();
        result?.Items?.forEach((item) => {
            expect(item.referenceId.id).toBe(populateFilter[0].filter.id);
        });
    });
});
